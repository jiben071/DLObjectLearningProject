...
- (void)install {
    ...
    if ([self supportsActiveProperty] && self.layoutConstraint) {//防止重复添加约束
    ...
    // alignment attributes must have a secondViewAttribute  对齐属性必须要一个第二属性
    // therefore we assume that is refering to superview  不然我们就假设是相对父视图
    ...
    if (existingConstraint) {
        ...
        [firstLayoutItem.mas_installedConstraints addObject:self];//mas_installedConstraints属性记录一下本安装的约束
    }
}

- (MASLayoutConstraint *)layoutConstraintSimilarTo:(MASLayoutConstraint *)layoutConstraint {
    // check if any constraints are the same apart from the only mutable property constant
    // 检查是否有任何约束与唯一的可变属性常量相同
