...
/**
 ...
 POPAnimation 抽象基类定义了动画的名字，开始时间，重复次数，是否自动倒退等，既可以使用delegate的方式来监听动画的状态，也支持使用block的方式。它是个抽象类，不是被直接使用的。
 POPAnimation相当于只提供对外的接口。 POP里面每个子类Animation都有一个State与它一一对应。
 */
